...
extern template std::unique_ptr<ocs2::SLQ<STATE_DIM, INPUT_DIM>> getSlq<QuadrupedInterface>(const QuadrupedInterface& quadrupedInterface,
                                                                                            const ocs2::SLQ_Settings& slqSettings);
extern template std::unique_ptr<ocs2::MPC_SLQ<STATE_DIM, INPUT_DIM>> getMpc<QuadrupedInterface>(
    const QuadrupedInterface& quadrupedInterface, const ocs2::MPC_Settings& mpcSettings, const ocs2::SLQ_Settings& slqSettings);
}  // namespace switched_model
