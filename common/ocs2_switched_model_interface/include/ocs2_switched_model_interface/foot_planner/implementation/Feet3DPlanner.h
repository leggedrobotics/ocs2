...
Feet3DPlanner<scalar_t>::Feet3DPlanner(const scalar_t& swingLegLiftOff, const scalar_t& swingTimeScale /*= 1.0*/)

    : swingLegLiftOff_(swingLegLiftOff), swingTimeScale_(swingTimeScale), phaseIDsStock_(0), eventTimes_(0) {}
...
Feet3DPlanner<scalar_t>::Feet3DPlanner(const Feet3DPlanner& rhs)
    ...
    : BASE(rhs), swingLegLiftOff_(rhs.swingLegLiftOff_), swingTimeScale_(rhs.swingTimeScale_), phaseIDsStock_(0), eventTimes_(0) {}
...
/******************************************************************************************************/
template <typename scalar_t>
void Feet3DPlanner<scalar_t>::setFootholdsPlan(const scalar_array_t& touchdownTimeStock,
...
/******************************************************************************************************/
template <typename scalar_t>
void Feet3DPlanner<scalar_t>::planSingleMode(const size_t& index, const size_array_t& phaseIDsStock, const scalar_array_t& eventTimes,
  ...
  if (phaseIDsStock_ != phaseIDsStock) {
    ...
    for (size_t j = 0; j < 4; j++) {
      BASE::updateFootSchedule(j, phaseIDsStock_, eesContactFlagStocks_[j], BASE::startTimesIndices_[j], BASE::finalTimesIndices_[j]);
    }
  }
  ...
  for (size_t j = 0; j < 4; j++) {
    ...
    if (eesContactFlagStocks_[j][index] == false) {
      ...
      plannedCPG[j]->set(touchdownTimeStock_[index], swingStartTime, swingFinalTime, touchdownPosStock_[index][j],
                         touchdownVelStock_[index][j], touchdownPosStock_[swingFinalIndex][j], adaptedLiftOff);
