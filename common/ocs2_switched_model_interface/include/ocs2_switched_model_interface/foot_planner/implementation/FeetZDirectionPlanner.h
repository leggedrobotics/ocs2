...
FeetZDirectionPlanner<scalar_t, cpg_t>::FeetZDirectionPlanner(const scalar_t& swingLegLiftOff, const scalar_t& swingTimeScale /*= *1.0*/,
...
/******************************************************************************************************/
template <typename scalar_t, class cpg_t>
void FeetZDirectionPlanner<scalar_t, cpg_t>::planSingleMode(const size_t& index, const size_array_t& phaseIDsStock,
                                                            const scalar_array_t& eventTimes, feet_cpg_ptr_t& plannedCPG) {
  ...
  for (size_t j = 0; j < 4; j++) {
    ...
    plannedCPG[j] = std::shared_ptr<cpg_t>(new cpg_t(swingLegLiftOff_, swingTimeScale_, liftOffVelocity_, touchDownVelocity_));
    ...
    if (eesContactFlagStocks_[j][index] == false) {
      ...
      if (swingStartIndex == -1) {
        ...
        for (size_t i = 0; i < phaseIDsStock_.size(); i++) std::cerr << "[" << i << "]: " << phaseIDsStock_[i] << ",  ";
        std::cerr << std::endl;
      ...
      plannedCPG[j]->set(swingStartTime, swingFinalTime, adaptedLiftOff);
