...
feet_array_t<vector3_s_t<SCALAR_T>> torqueApproximation(const joint_coordinate_s_t<SCALAR_T>& jointPositions,
  ...
  feet_array_t<vector3_s_t<SCALAR_T>> torques;
  for (int leg = 0; leg < NUM_CONTACT_POINTS; ++leg) {
    ...
    torques[leg] = -b_baseToFootJacobianBlock.bottomRows(3).transpose() * contactForcesInBase[leg];
  }
  return torques;
}

template feet_array_t<vector3_s_t<scalar_t>> torqueApproximation<scalar_t>(const joint_coordinate_s_t<scalar_t>& jointPositions,
                                                                           const feet_array_t<vector3_s_t<scalar_t>>& contactForcesInBase,
                                                                           const KinematicsModelBase<scalar_t>& kinematics);
template feet_array_t<vector3_s_t<ad_scalar_t>> torqueApproximation<ad_scalar_t>(
