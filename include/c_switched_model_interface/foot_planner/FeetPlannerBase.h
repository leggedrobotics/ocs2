...
protected:
	 ...
	 * @param [out] contactFlagStock: The sequence of the cantact status for the requested leg.
	 ...
	 *
	 * @param [in] phaseIDsStock: The sequence of the motion phase IDs.
	 * @param [out] contactFlagStock: The sequence of the cantact status for each leg.
	 */
	void extractContactFlags(const size_array_t& phaseIDsStock,
			std::array<bool_array_t,4>& contactFlagStock) const;

	/**
	 * Finds the take-off and touch-down times indeces for a specific leg.
	 *
	 * @param [in] index: The index of the subsystem for which the take-off and touch-down times indeces should be found.
