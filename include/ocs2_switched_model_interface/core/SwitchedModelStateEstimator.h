...
class SwitchedModelStateEstimator {
 public:
  ...
  SwitchedModelStateEstimator(const ComModelBase<JOINT_COORD_SIZE>& comModel) : comModelPtr_(comModel.clone()) {}
  ...
  void estimateComkinoModelState(const rbd_model_state_t& rbdState, comkino_model_state_t& comkinoState) {
    ...
    comkinoState << comState, rbdState.template segment<JOINT_COORD_SIZE>(6);
  }
  ...
  void estimateFullModelState(const rbd_model_state_t& rbdState, full_model_state_t& fullState) {
    ...
    fullState << comState, rbdState.template segment<JOINT_COORD_SIZE>(6), rbdState.template tail<JOINT_COORD_SIZE>();
  }
  ...
  void estimateComState(const rbd_model_state_t& rbdState, com_model_state_t& comState) {
    ...
    comState << rbdState.template head<3>(), rbdState.template segment<3>(3) + o_R_b * b_r_com, comLocalVelocities;
  }
   ...
   */
  void estimateRbdModelState(const comkino_model_state_t& comkinoState, const joint_coordinate_t& dqJoints, rbd_model_state_t& rbdState) {
