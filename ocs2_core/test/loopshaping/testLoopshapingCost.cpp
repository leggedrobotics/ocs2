...
TYPED_TEST(TestFixtureLoopShapingCost, testApproximation) {
...
TYPED_TEST(TestFixtureLoopShapingCost, testStateDerivative) {
  ...
  for (int i = 0; i < n_random_tests; i++) {
    ...
    // Reevaluate at disturbed state
    typename TestFixtureLoopShapingCost<TypeParam>::scalar_t L_disturbance;
    this->testLoopshapingCost->setCurrentStateAndControl(this->t, this->x_ + x_disturbance, this->u_);
    this->testLoopshapingCost->getIntermediateCost(L_disturbance);
...
TYPED_TEST(TestFixtureLoopShapingCost, testDerivativeInput) {
  ...
  for (int i = 0; i < n_random_tests; i++) {
    ...
    // Reevaluate at disturbed state
    typename TestFixtureLoopShapingCost<TypeParam>::scalar_t L_disturbance;
    this->testLoopshapingCost->setCurrentStateAndControl(this->t, this->x_, this->u_ + u_disturbance);
    this->testLoopshapingCost->getIntermediateCost(L_disturbance);
