...
public:
	...
	systemDynamics* clone() const final
	{
...
public:
	...
	systemDerivative* clone() const final
	{
		return new systemDerivative(*this);
	}
...
public:
	...
	systemCost(OverallReference ref, state_matrix_t Q, input_matrix_t R,state_matrix_t P,
			   state_vector_t xNom,input_vector_t uNom, state_vector_t xFin)
	:BASE(Q,R,xNom,uNom,P,xFin),ref_(ref),xFin_(xFin){}

	~systemCost() = default;

	systemCost* clone() const final{
		return new systemCost(*this);
	}
