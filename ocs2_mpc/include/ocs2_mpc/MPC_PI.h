...
public:
	...
	typedef MPC_BASE<STATE_DIM, INPUT_DIM, LOGIC_RULES_T> BASE;
        ...
        MPC_PI(typename dynamics_t::Ptr dynamics, typename cost_t::Ptr cost, const MPC_Settings& settings) : BASE(scalar_array_t{0.0, 1.0}, settings) {
          piSolverPtr_.reset(new solver_t(dynamics, cost));
	 ...
	 */
        virtual solver_base_t* getSolverPtr() override {
          return piSolverPtr_.get() ;
        } // TODO(jcarius) should this happen in the base class
