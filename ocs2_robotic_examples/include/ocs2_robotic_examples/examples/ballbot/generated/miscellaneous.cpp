...
iit::rbd::Vector3d iit::Ballbot::getWholeBodyCOM(
    ...
    const HomogeneousTransforms& ht)
{
    ...
    tmpSum += inertiaProps.getMass_dummy_ball1() *
            ( iit::rbd::Utils::transform(tmpX, inertiaProps.getCOM_dummy_ball1()));
    
    tmpX = tmpX * ht.fr_dummy_ball1_X_fr_ball;
    tmpSum += inertiaProps.getMass_ball() *
            ( iit::rbd::Utils::transform(tmpX, inertiaProps.getCOM_ball()));
    
    tmpX = tmpX * ht.fr_ball_X_fr_dummy_base1;
    tmpSum += inertiaProps.getMass_dummy_base1() *
            ( iit::rbd::Utils::transform(tmpX, inertiaProps.getCOM_dummy_base1()));
    
    tmpX = tmpX * ht.fr_dummy_base1_X_fr_dummy_base2;
    tmpSum += inertiaProps.getMass_dummy_base2() *
            ( iit::rbd::Utils::transform(tmpX, inertiaProps.getCOM_dummy_base2()));
    
    tmpX = tmpX * ht.fr_dummy_base2_X_fr_base;
    tmpSum += inertiaProps.getMass_base() *
            ( iit::rbd::Utils::transform(tmpX, inertiaProps.getCOM_base()));
