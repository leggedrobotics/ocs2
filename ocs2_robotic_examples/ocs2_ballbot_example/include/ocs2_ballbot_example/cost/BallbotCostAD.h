...
class BallbotCostAD : public ocs2::CostFunctionBaseAD<BallbotCostAD, ballbot::STATE_DIM_, ballbot::INPUT_DIM_> {
 public:
   ...
   */
  BallbotCostAD(const state_matrix_t& Q, const input_matrix_t& R, const state_vector_t& x_nominal, const input_vector_t& u_nominal,
                const state_matrix_t& Q_final, const state_vector_t& x_final, const bool& dynamicLibraryIsCompiled = false)
      : BASE(dynamicLibraryIsCompiled), Q_(Q), R_(R), x_nominal_(x_nominal), u_nominal_(u_nominal), Q_final_(Q_final), x_final_(x_final) {}
  ...
  void intermediateCostFunction(const SCALAR_T& time, const Eigen::Matrix<SCALAR_T, ballbot::STATE_DIM_, 1>& state,
    ...
    Eigen::Matrix<SCALAR_T, 3, 1> positionWorldToControlFrameInWorldFrame =
        homogeneousTransformWorldToControlFrame.template topRightCorner<3, 1>();
    ...
    Eigen::Matrix<SCALAR_T, 3, 1> linearVelocityWorldToControlFrameInWorldFrame =
        jacobianWorldToControlFrameInWorldFrame.template bottomRows<3>() * state.template segment<5>(5);
    ...
    stateDeviation.template segment<2>(5) =
        stateDesired.template segment<2>(5) - linearVelocityWorldToControlFrameInWorldFrame.template head<2>();

    // compute value of the cost function
    costValue =
        0.5 * stateDeviation.dot(Q_.template cast<SCALAR_T>() * stateDeviation) + 0.5 * input.dot(R_.template cast<SCALAR_T>() * input);
  }
   ...
   */
  template <typename SCALAR_T>
  void terminalCostFunction(const SCALAR_T& time, const Eigen::Matrix<SCALAR_T, ocs2::ballbot::STATE_DIM_, 1>& state,
    ...
    desiredQuaternionBaseToWorld.w() = SCALAR_T(1.0);
    desiredQuaternionBaseToWorld.vec().setZero();
   ...
   */
  logic_variable_t getlogicVariables(scalar_t time) { return logic_variable_t::Zero(); }
