...
TEST(BallbotRaisimConversions, AngularVelocities) {
  ...
  auto eulerAnglesDerivative =
      ocs2::ballbot::BallbotRaisimConversions::angularVelocityInWorldToEulerAngleDerivatives(zeroEulerAngles, omega_base_inWorld);
  EXPECT_TRUE(eulerAnglesDerivative.isZero());

  omega_base_inWorld = 0.1 * Eigen::Vector3d::UnitZ();
  eulerAnglesDerivative =
      ocs2::ballbot::BallbotRaisimConversions::angularVelocityInWorldToEulerAngleDerivatives(zeroEulerAngles, omega_base_inWorld);
...
TEST(BallbotRaisimConversions, AngularVelocitiesSelfConsistency) {
  for (int i = 0; i < 10; i++) {
    ...
    auto eulerAnglesDerivative =
        ocs2::ballbot::BallbotRaisimConversions::angularVelocityInWorldToEulerAngleDerivatives(eulerAngles, omega_base_inWorld);
    auto omega_base_inWorld_calc =
        ocs2::ballbot::BallbotRaisimConversions::eulerAngleDerivativesToAngularVelocityInWorld(eulerAngles, eulerAnglesDerivative);
    auto eulerAnglesDerivative_calc =
        ocs2::ballbot::BallbotRaisimConversions::angularVelocityInWorldToEulerAngleDerivatives(eulerAngles, omega_base_inWorld_calc);
