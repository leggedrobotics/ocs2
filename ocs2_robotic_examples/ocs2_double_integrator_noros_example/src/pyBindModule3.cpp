...
// convenience macro to bind robot interface with all required vectors. LIB_NAME must match target name in CMakeLists
#define CREATE_ROBOT_PYTHON_BINDINGS(PY_INTERFACE, LIB_NAME)\
  ...
  PYBIND11_MODULE(LIB_NAME, m) {\
    ...
    VECTOR_TYPE_BINDING(PY_INTERFACE::input_vector_array_t, "input_vector_array_t")\
    VECTOR_TYPE_BINDING(PY_INTERFACE::state_matrix_array_t, "state_matrix_array_t")\
    /* bind the actual mpc interface */\
    pybind11::class_<PY_INTERFACE>(m, "mpc_interface")\
        .def(pybind11::init<const std::string&, bool>())\
        .def_property_readonly_static("STATE_DIM", [](pybind11::object) {return static_cast<int>(PY_INTERFACE::dim_t::DIMS::STATE_DIM_);})\
        ...
        .def("setFlowMapDerivativeStateAndControl", &PY_INTERFACE::setFlowMapDerivativeStateAndControl, "t"_a, "x"_a.noconvert(), "u"_a.noconvert())\
...
CREATE_ROBOT_PYTHON_BINDINGS(DoubleIntegratorPyBindings, DoubleIntegratorPyBindings3)
