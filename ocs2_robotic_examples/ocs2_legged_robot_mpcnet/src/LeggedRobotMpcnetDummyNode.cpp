...
int main(int argc, char** argv) {
  ...
  if (programArgs.size() < 6) {
    throw std::runtime_error(
        "No robot name, config folder, target command file, description name, or policy file path specified. Aborting.");
  }
  ...
  // rollout
  std::unique_ptr<RolloutBase> rolloutPtr(leggedRobotInterface.getRollout().clone());
  ...
  std::shared_ptr<LeggedRobotVisualizer> leggedRobotVisualizerPtr(new LeggedRobotVisualizer(
