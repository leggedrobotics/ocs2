...
std::pair<ocs2::TargetTrajectories, Gait> readCartesianMotion(const CsvData& csvData, scalar_t dt) {
  ...
  for (const auto& dataLine : csvData.data) {
    ...
    // State trajectory
    targetTrajectories.stateTrajectory.push_back(vector_t(2 * BASE_COORDINATE_SIZE + 6 * NUM_CONTACT_POINTS));
    targetTrajectories.stateTrajectory.back() << eulerXYZ, basePositionInWorld, baseAngularvelocityInBase, baseLinearvelocityInBase,
        feetPositions, feetVelocities;

    // Input trajectory
    targetTrajectories.inputTrajectory.push_back(vector_t(3 * NUM_CONTACT_POINTS));
