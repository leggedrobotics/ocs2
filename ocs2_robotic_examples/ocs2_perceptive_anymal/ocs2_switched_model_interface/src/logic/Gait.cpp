...
bool isValidGait(const Gait& gait) {
  ...
  validGait &=
      std::is_sorted(gait.eventPhases.begin(), gait.eventPhases.end());
...
int getModeIndexFromPhase(scalar_t phase, const Gait& gait) {
  ...
  auto firstLargerValueIterator = std::upper_bound(
      gait.eventPhases.begin(), gait.eventPhases.end(), phase);
