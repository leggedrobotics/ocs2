...
int main( int argc, char* argv[] )
{
	...
	Eigen::Matrix<double, 12, 1> initJoints = initRbdState_.segment(6, 12);
  	...
  	quadrupedKeyboard::scalar_array_t command_limits = quadrupedKeyboard::scalar_array_t{2.0, 2.0, 0.0, 0.0, 0.0, 1.0, 2.0, 2.0, 2.0, 40.0, 40.0, 360.0};
	switched_model::TargetTrajectories_Keyboard_Quadruped<double, 24, 24> targetPoseCommand(argc, argv, "anymal",
																							initZHeight,
																							initJoints,
																							modelSettings.targetDisplacementVelocity_,
																							modelSettings.targetRotationVelocity_,
																							command_limits,
																							command_mode);
